...
def read_upcoming_events(db: Session = Depends(get_db)):
    ...
    return db.query(models.Event).options(joinedload(models.Event.championship)).filter(models.Event.start_date.between(today_start, one_week_from_now)).order_by(models.Event.start_date).all()
